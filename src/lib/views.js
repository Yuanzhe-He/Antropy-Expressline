// Server-side view layer: the request-shaping + EJS render-prep helpers for the
// workbench (handover/customs/inland/quote) and admin pages, plus the small
// shared accessors (baseView, getModuleData, loadShippingData, redirectWithFlash).
// Pure data/presentation transforms — extracted from server.js as a pure move
// (bodies are byte-for-byte the originals). Layer: lib. createApp assembles these
// into the per-route ctx objects.
//
// Public API: see module.exports at the bottom.
...
const { DEFAULT_MODULE_KEY, normalizeModuleKey } = require("./modules");
const {
