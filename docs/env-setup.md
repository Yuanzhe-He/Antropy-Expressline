...
- `PDF_BROWSER_IDLE_TIMEOUT_MS`：最后一个 PDF 导出完成后，浏览器空闲多久自动关闭。默认 `180000`（3 分钟）；允许 1,000–3,600,000 毫秒，无效值使用默认值。并发导出结束之前不会触发空闲关闭，后续导出会重新启动浏览器。
- `SESSION_IDLE_TTL_MS`：服务器端会话的空闲有效期，默认 `86400000`（24 小时）。如 Cookie 自身更早到期，则使用更早时间。
- `SESSION_MAX_ENTRIES`：单个应用进程最多保留多少会话，默认 `10000`；超出后清理最久未使用的会话。
- `SESSION_CLEANUP_INTERVAL_MS`：会话过期清理间隔，默认 `60000`（1 分钟）。

会话仍保存在应用进程内，重启后会清空，与原有 MemoryStore 一致。浏览器 Cookie 属性、语言、表单及提示消息的用途不变；`/healthz` 健康检查不再创建会话。这些变量应使用正整数，不涉及数据库迁移或新增付费存储。
